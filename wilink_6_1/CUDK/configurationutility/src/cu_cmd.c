...
VOID CuCmd_ConfigBtCoe(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    ...
    if( nParms != NUM_OF_CONFIG_PARAMS_IN_SG )
    {
        ...
        os_error_printf(CU_MSG_INFO2, (PS8)"Param 37 - coexTempParam1 \n");
		os_error_printf(CU_MSG_INFO2, (PS8)"Param 38 - coexTempParam2 \n");
		os_error_printf(CU_MSG_INFO2, (PS8)"Param 39 - coexTempParam3 \n");
		os_error_printf(CU_MSG_INFO2, (PS8)"Param 40 - coexTempParam4 \n");
...
void FillMACAddressToNVS(THandle hCuCmd, FILE *nvsBinFile)
{
	CuCmd_t* pCuCmd = (CuCmd_t*)hCuCmd;
	TMacAddr Mac;
	U8	lengthToSet;
...
/****************************************************************************/
/*  Function:		FillTXParameterToNVS									*/
/****************************************************************************/
/*  Description:	Fill TX parameters to the NVS							*/ 
/*  Parameters:		nvsBinFile - NVS bin file								*/
/*					nvsPtr - NVS pointer									*/
/*  Return Value:	True - file created, False - otherwise					*/
/****************************************************************************/
void FillTXParameterToNVS(FILE	*nvsBinFile, const TTestCmdP2GCal *nvsPtr)
{
	TTestCmdP2GCal	*ptgStruct	= (TTestCmdP2GCal *)nvsPtr;
	U16			tlvLength;
	U16			index;
	U8			valueToSet;
	...
	/* TX BiP type */
	valueToSet = eNVS_RADIO_TX_PARAMETERS;
    os_fwrite(&valueToSet, sizeof(U8), 1, nvsBinFile);

	/* TX Bip Length */
	tlvLength = NVS_TX_PARAM_LENGTH;
    os_fwrite(&tlvLength, sizeof(U16), 1, nvsBinFile);

	for (index = 0; index < NVS_TX_PARAM_LENGTH; index++)
	{
	 valueToSet = ptgStruct->oNvsStruct.Buffer[index];
     os_fwrite(&valueToSet, sizeof(U8), 1, nvsBinFile);			
	}
}
...
/****************************************************************************/
/*  Function:		FillOldRXParameterToNVS									*/
/****************************************************************************/
/*  Description:	Fill old RX parameters to the NVS						*/ 
/*  Parameters:		nvsBinFile - NVS bin file								*/
/*					buffer - buffer of uin8 values							*/
/*					maxLengthOfBuffer - maximum length of all the buffer	*/
/*  Return Value:	True - file created, False - otherwise					*/
/****************************************************************************/
void FillOldRXParameterToNVS(FILE *nvsBinFile, const U8	*buffer)
{
	U32		index;
	U32		paramStartIndex = NVS_PRE_PARAMETERS_LENGTH + NVS_RX_PARAM_INDEX;
	U32		paramEndIndex = paramStartIndex + NVS_RX_PARAM_LENGTH;
	U8		valueToSet;
	U16		tlvLength;
		
	/* RX BiP type */
	valueToSet = eNVS_RADIO_RX_PARAMETERS;
    os_fwrite(&valueToSet, sizeof(U8), 1, nvsBinFile);

	/* RX Bip Length */
	tlvLength = NVS_RX_PARAM_LENGTH;
    os_fwrite(&tlvLength, sizeof(U16), 1, nvsBinFile);

	for (index = paramStartIndex; 
	    (index < paramEndIndex);
        index++)
	{
...
/****************************************************************************/
/*  Function:		FillRXParameterToNVS									*/
/****************************************************************************/
/*  Description:	Fill old TX parameters to the NVS						*/ 
/*  Parameters:		nvsBinFile - NVS bin file								*/
/*					buffer - buffer of uin8 values							*/
/*					maxLengthOfBuffer - maximum length of all the buffer	*/
/*  Return Value:	True - file created, False - otherwise					*/
/****************************************************************************/
void FillRXParameterToNVS(FILE *nvsBinFile, const RadioRxPltCal *nvsPtr)
{
    RadioRxPltCal	*ptgStruct	= (RadioRxPltCal *)nvsPtr;
	U32		index;
   	U8		valueToSet;
	U16		tlvLength;
...
/****************************************************************************/
/*  Function:		FillOldTXParameterToNVS									*/
/****************************************************************************/
/*  Description:	Fill old TX parameters to the NVS						*/ 
/*  Parameters:		nvsBinFile - NVS bin file								*/
/*					buffer - buffer of uin8 values							*/
...
/*  Return Value:	True - file created, False - otherwise					*/
/****************************************************************************/
void FillOldTXParameterToNVS(FILE *nvsBinFile, const U8	*buffer)
{
    U32		index ;
   	U8		valueToSet;
	U16		tlvLength;
    U32		paramStartIndex = NVS_PRE_PARAMETERS_LENGTH + NVS_TX_PARAM_INDEX;
    U32		paramEndIndex = paramStartIndex + NVS_TX_PARAM_LENGTH;
    ...
    for (index = paramStartIndex; index < paramEndIndex; index++)
	{
	  valueToSet = buffer[index];
      os_fwrite(&valueToSet, sizeof(U8), 1, nvsBinFile);			
	}
...
/****************************************************************************/
/*  Function:		FillDefaultTXParameterToNVS								*/
/****************************************************************************/
/*  Description:	Set 0 in all tX parameters								*/ 
/*  Parameters:		nvsBinFile - NVS bin file								*/
/*  Return Value:	True - file created, False - otherwise					*/
/****************************************************************************/
void FillDefaultTXParameterToNVS(FILE *nvsBinFile)
{
	S8	typeValue = eNVS_RADIO_TX_PARAMETERS;
	U16	tlvLength = NVS_TX_PARAM_LENGTH;
	U8	valueToSet = 0;
	U32	txParamIndex;

	/* TX type */
    os_fwrite(&typeValue, sizeof(U8), 1, nvsBinFile);

	/* TX length; */
    os_fwrite(&tlvLength, sizeof(U16), 1, nvsBinFile);

	for (txParamIndex = 0; txParamIndex < tlvLength; txParamIndex++)
	{		
      os_fwrite(&valueToSet, sizeof(U8), 1, nvsBinFile);
	}
}
...
/****************************************************************************/
/*  Function:		FillDefaultRXParameterToNVS								*/
/****************************************************************************/
/*  Description:	Set 0 in all RX parameters								*/ 
/*  Parameters:		nvsBinFile - NVS bin file								*/
/*  Return Value:	True - file created, False - otherwise					*/
/****************************************************************************/
void FillDefaultRXParameterToNVS(FILE *nvsBinFile)
{
	S8	typeValue = eNVS_RADIO_RX_PARAMETERS;
	U16	lengthValue = NVS_RX_PARAM_LENGTH;
	U8	valueToSet = 0;
	S8	rxParamIndex;
	...
	/* RX length; */
    os_fwrite(&lengthValue, sizeof(U16), 1, nvsBinFile);
...
/****************************************************************************/
/*  Function:		WriteEndNVSParam										*/
/****************************************************************************/
/*  Description:	Write end of NVS										*/ 
/*  Parameters:		nvsBinFile - NVS bin file								*/
/*  Return Value:	None													*/
/****************************************************************************/
void WriteEndNVSParam(FILE *nvsBinFile)
{
	U16			lengthToSet;
	U8			valueToSet;
...
/****************************************************************************/
/*  Function:		FillVersionToNVS										*/
/****************************************************************************/
/*  Description:	Fill version to the NVS									*/ 
/*  Parameters:		nvsBinFile - NVS bin file								*/
/*					oldVersionValid - old verson parameter valide			*/
/*					buffer - buffer of uin8 values							*/
/*					maxLengthOfBuffer - maximum length of all the buffer	*/
/*  Return Value:	None													*/
/****************************************************************************/
...
/****************************************************************************/
void FillVersionToNVS(FILE			*nvsBinFile,
									 S8		*versionStr,
									 const TI_BOOL	oldVersionValid,
									 const U8	    *buffer,
									 const U32  	maxLengthOfBuffer)
{
#define NUMBER_OF_DIGIT_VERISON		3
	U8		version[NUMBER_OF_DIGIT_VERISON];
	U8		charToVersion = 0;
	U8		tempStr[MAX_NVS_VERSION_LENGTH];
	U8		numberOfVersionDigit;
	U8		valueToSet;
	U16		lengthToSet;
	U32		index;
	U32		lengthOfStructStr;
	U32		startParamIndex;
	U32  	endVersionParamIndex;
	U8		indexForDefaultVersion;

	/* version type */
	valueToSet = eNVS_VERSION;
    fwrite(&valueToSet, sizeof(U8), 1, nvsBinFile);

	/* version length */
	lengthToSet = NUMBER_OF_DIGIT_VERISON;
    os_fwrite(&lengthToSet, sizeof(U16), 1, nvsBinFile);	

	if (versionStr != NULL)
	{
		lengthOfStructStr = os_strlen(versionStr);

		for (numberOfVersionDigit = 0, charToVersion = 0, index = 0; 
			 (index < MAX_NVS_VERSION_LENGTH) && (numberOfVersionDigit < NUMBER_OF_DIGIT_VERISON); charToVersion = 0, index++)
		{
			while ((versionStr[index] != '.') && (index < lengthOfStructStr))
			{
				if ((versionStr[index] < '0') || (versionStr[index] > '9'))
				{
					index++;
				}
				else
				{
					tempStr[charToVersion++] = versionStr[index++];				
				}
			}			

			tempStr[charToVersion] = '\0';

			if (0 == os_strlen((PS8)tempStr))
			{
				for (valueToSet = 0, indexForDefaultVersion = 0; indexForDefaultVersion < NUMBER_OF_DIGIT_VERISON - numberOfVersionDigit; indexForDefaultVersion++)
				{
                   os_fwrite(&valueToSet, sizeof(U8), 1, nvsBinFile);
				}
			...
			version[numberOfVersionDigit] = (U8)os_atoi((PS8)tempStr);
            os_fwrite(&(version[numberOfVersionDigit]), sizeof(U8), 1, nvsBinFile);

			numberOfVersionDigit++;
		}
	}
	else 
	{
		if (oldVersionValid)
		{
			...
			startParamIndex			= NVS_PRE_PARAMETERS_LENGTH + NVS_VERSION_PARAMETER_INDEX;
			endVersionParamIndex	= startParamIndex + NUMBER_OF_DIGIT_VERISON;

			if (maxLengthOfBuffer >= endVersionParamIndex)
			{
				for (index = startParamIndex; index < endVersionParamIndex; index++)
				{
					valueToSet = buffer[index];
                    os_fwrite(&valueToSet, sizeof(U8), 1, nvsBinFile);
				}
		...
		else
		{
			...
			for (index = 0; index < NUMBER_OF_DIGIT_VERISON; index++)
			{
                os_fwrite(&valueToSet, sizeof(U8), 1, nvsBinFile);
			}
		}
...
VOID build_nvs_file(THandle hCuCmd, TTestCmd data, S8 updatedProtocol)
{
#ifdef _WINDOWS
    PS8 nvs_file_name = (PS8)"/windows/nvs_map.bin";
#else
    PS8 nvs_file_name = (PS8)"./nvs_map.bin";
#endif /*_WINDOWS*/
	FILE *nvsBinFile = NULL;
	S8 index;
	U8 valueToSet = 0;
	S8 nvsFileValid = TRUE;
    uint8 	Buffer[MAX_TLV_LENGTH];
    uint8   uTxTypeIndexValue;
    uint8	uRrxTypeIndexValue;

    
    os_memset (Buffer, 0, MAX_TLV_LENGTH);

    if (NULL == (nvsBinFile = os_fopen (nvs_file_name, OS_FOPEN_READ_BINARY)))
    {
       nvsFileValid = FALSE;        
    }
    else
    {
        os_fread(Buffer,1, MAX_TLV_LENGTH, nvsBinFile);
        os_fclose(nvsBinFile);

        uTxTypeIndexValue = Buffer[NVS_PRE_PARAMETERS_LENGTH + NVS_TX_TYPE_INDEX];
        uRrxTypeIndexValue = Buffer[NVS_PRE_PARAMETERS_LENGTH + NVS_RX_TYPE_INDEX];

        /* if the NVS includes just the MAC preamble (the firest 24B) don't use the Tx and RX values */
        if ((uTxTypeIndexValue != eNVS_RADIO_TX_PARAMETERS) || (uRrxTypeIndexValue != eNVS_RADIO_RX_PARAMETERS))
		{
			nvsFileValid = FALSE;
		}
    }
	...
	/* open NVS file */
	if (NULL == (nvsBinFile = os_fopen (nvs_file_name, OS_FOPEN_WRITE_BINARY)))
	{
		os_error_printf(CU_MSG_INFO2, (PS8)"Open NVS file failed\n");        
		return;
	}

	FillMACAddressToNVS(hCuCmd, nvsBinFile);
	...
	if(NVS_FILE_TX_PARAMETERS_UPDATE == updatedProtocol)
	{
		...
		FillTXParameterToNVS(nvsBinFile, &data.testCmd_u.P2GCal);
		if (nvsFileValid)
		{
    ...
    else  /* Fill new RX BiP values*/
    {
       ...
       /* Fill new RX BiP values*/
        FillRXParameterToNVS(nvsBinFile, &data.testCmd_u.RxPlt);

    }


    /*Fill the NVS version to the NVS*/
	FillVersionToNVS(nvsBinFile,
						   data.testCmd_u.P2GCal.oNvsVersion,
						   nvsFileValid,
						   data.testCmd_u.P2GCal.oNvsStruct.Buffer,
						   data.testCmd_u.P2GCal.oNvsStruct.Length);

	/* End of NVS*/
	WriteEndNVSParam(nvsBinFile);

    
#if 0
	/* Save NVS Results in NVS file */
	if (data.testCmd_u.P2GCal.oNvsStruct.Length != os_fwrite(data.testCmd_u.P2GCal.oNvsStruct.Buffer, sizeof(U8),data.testCmd_u.P2GCal.oNvsStruct.Length,nvs_file))
	{
		os_error_printf(CU_MSG_INFO2, (PS8)"Writing to NVS file failed\n"); 
	}
#endif
...
/*	for (i=0;i<data.testCmd_u.P2GCal.oNvsStruct.Length;i++) {
		os_error_printf(CU_MSG_INFO2, (PS8)"nvs buffer[%d]=%x\n, ", i, data.testCmd_u.P2GCal.oNvsStruct.Buffer[i]);
	} */
	
    build_nvs_file(hCuCmd, data, NVS_FILE_TX_PARAMETERS_UPDATE);
}
...
VOID CuCmd_BIP_StartRxBIP(THandle hCuCmd, ConParm_t parm[], U16 nParms)
{
    ...
    build_nvs_file(hCuCmd, data, NVS_FILE_RX_PARAMETERS_UPDATE);
